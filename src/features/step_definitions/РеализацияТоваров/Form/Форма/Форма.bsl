...
Процедура ДокументПроведен() Экспорт
	СообщенияПользователю = КонтекстСохраняемый.ТестовоеПриложение.ПолучитьАктивноеОкно().ПолучитьТекстыСообщенийПользователю();
	Для каждого Сообщение Из СообщенияПользователю Цикл
		Если СтрНайти(Сообщение, "Не хватает") > 0 Тогда 
...
Процедура ДокументНеПроведен() Экспорт
	ЕстьСообщениеобОшибке = Ложь;
	СообщенияПользователю = КонтекстСохраняемый.ТестовоеПриложение.ПолучитьАктивноеОкно().ПолучитьТекстыСообщенийПользователю();
	Для каждого Сообщение Из СообщенияПользователю Цикл
		Если СтрНайти(Сообщение, "Не хватает") > 0 Тогда 
			ЕстьСообщениеобОшибке = Истина;
		КонецЕсли;
	КонецЦикла;
	Если Не ЕстьСообщениеобОшибке Тогда 
		ВызватьИсключение "Документ проведен, хотя товара на остатке нет";
	КонецЕсли;
КонецПроцедуры
